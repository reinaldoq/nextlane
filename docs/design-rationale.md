...
**Known limitation (scoped honestly).** The current check enforces *two* machine-checked facts: (a) phase 1's gate has a failing `pytest` step **and** a change under `tests/`/`web/e2e/` (a genuine failing test was added), and (b) phase 2's *full* gate is green. What it does **not** yet pin is that the *same* reproduction test is the one that goes red→green: a cooperative agent writes a real regression test, but an adversarial or careless phase-2 session that rewrote or deleted the phase-1 test could still reach `repro_confirmed=True`. The honest framing is that we prove "a failing test existed before the fix and the suite is green after," not "this specific test reproduces the reported bug and survives." The hardening path is to capture phase 1's failing test node-ids and re-assert those exact ids transition red→green in phase 2 (and still exist) — deferred over parsing pytest node-ids under a time box, and called out here rather than papered over. This is the same "valid-failing-test generation is the real bottleneck" ceiling the research names.
